 ...
 */
 ...
 public class LexUtil {
     ...
     //对ICTCLAS进行必要的初始化
     private final static String CONFIG_PATH  = "C:/ictcals/";
     ...
     public static List<String> getWords(byte[] docData) {
         ...
         try {
			...
			if (ict.ICTCLAS_Init(argu.getBytes("GB2312"),0) == false)
			{
				throw new RuntimeException("ICTCLAS初始化失败");
			}
			/*if(false == ict.ICTCLAS_Init(CONFIG_PATH.getBytes("GB2312"),1)) {
			    throw new RuntimeException("ICTCLAS初始化失败");
			}*/
			byte[] results = ict.nativeProcAPara(docData);

...
<head>
	...
	<body class="list">
		...
		<div class="body articleCategory">
			<form id="listForm" action="menu/list" method="post">
				<div class="listBar">
					<input type="button" class="formButton" onclick="location.href='menu/add'" value="添加分类" hidefocus="">
				</div>
						...
						<tr grade="0">
							...
							<td>
								<a href="/shop/article_list/about_us.htm" target="_blank" title="浏览">[浏览]</a>
									<a href="menu/delete/${entry.id}" class="deleteArticleCategory" title="删除">[删除]</a>
								<a href="menu/edit/${entry.id}" title="编辑">[编辑]</a>
							</td>
						</tr>
						<c:forEach items="${entry.children}" var="child">
							<tr grade="1">
								...
								<td>
									...
									<a href="menu/edit/${child.id}" title="编辑">[编辑]</a>
								</td>
							</tr>
							<c:forEach items="${child.children}" var="grandchild">
							<tr grade="2">
								<td class="articleCategoryName">
									<span class="pointer category" style="margin-left: 40px;">
										${grandchild.name}
									</span>
								</td>
								<td>
								...
								<td>
									${grandchild.orderList}
								</td>
								<td>
									<a href="/shop/article_list/terms.htm" target="_blank" title="浏览">[浏览]</a>
										<a href="/category/delete/${grandchild.id}" class="deleteArticleCategory" title="删除">[删除]</a>
									<a href="menu/edit/${grandchild.id}" title="编辑">[编辑]</a>
								</td>
							</tr>
						</c:forEach>

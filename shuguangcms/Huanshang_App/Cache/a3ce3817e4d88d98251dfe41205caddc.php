...
<link rel="stylesheet" href="/Huanshang_App/Tpl/default/Public/css/category.css"/>
</head>
<body>
<!-- header start -->
<!-- 
...
<div id="mainbody">
 <div class="left_side">
   ...
   <h1>产品分类</h1>
    <div class="category">
     <ul>
     	<?php if(is_array($cate)): ?><?php $i = 0;?><?php $__LIST__ = $cate?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = (($i % 2 )==0)?><li class="left_side_line"><a href="/index.php/Product/index/module/<?php echo ($module); ?>/id/<?php echo (is_array($vo)?$vo["id"]:$vo->id); ?>"><?php echo (is_array($vo)?$vo["title"]:$vo->title); ?></a></li><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
     </ul>
    </div>
   </div>
   <div class="left_side_bottom"></div>
	...
	<div class="right-side">
		...
		<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = (($i % 2 )==0)?><div class="i_w_f <?php if($i == 1): ?>first<?php endif; ?>">
				<div class="hd"></div>
				<div class="bd">
					<ul class="pic">
						<li type="goods" style="width: 200px; height: 200px">
							<a target="_blank" href="/index.php/Product/<?php echo (is_array($vo)?$vo["id"]:$vo->id); ?>.html" > 
								<img src="/attachments/product/<?php echo (is_array($vo)?$vo["attachpath"]:$vo->attachpath); ?>/<?php echo (is_array($vo)?$vo["attachthumb"]:$vo->attachthumb); ?>" alt="" title="" width="196px" height="194px"/>
							</a>
						</li>
					</ul>
				</div>
				...
				<div class="who_share">
					...
					<a href="/index.php/Product/<?php echo (is_array($vo)?$vo["id"]:$vo->id); ?>.html"><?php echo (is_array($vo)?$vo["subject"]:$vo->subject); ?></a>
					</div>
					<div class="ft"></div>
				</div>
				<?php if((is_array($vo)?$vo["module"]:$vo->module) == 1): ?><div class="band onsale"></div><?php endif; ?>
			</div><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
		<div class="tdpage"><?php echo ($page); ?></div>
		</div>
	</div>
</div>

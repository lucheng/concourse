    ...
    if ($step == 5)
    {
        ...
        if ($setupsta == 0)
        {
            ...
            */
            ...
            $errstr = GetBackAlert("写入配置 /database.inc.php 失败，请检查 /database.inc.php 文件夹是否可读写！");
            $fp = fopen(ROOTDIR."/database.inc.php", "w+") or die($errstr);
            flock($fp, 2);
            fwrite($fp, $strConfig);
            fclose($fp);
            ...
            while (!feof($fp))
            {
                ...
                {
                    $query .= $line;
                    $query = str_replace('t_', $dbPrefix, $query);

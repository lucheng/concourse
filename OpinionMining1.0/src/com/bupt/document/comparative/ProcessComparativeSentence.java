 ...
 *      比较句情感得分集
 */
	public  static LinkedList<SentiResult> processcomparativesentence(String keyword,String str){
    	...
    	if(keyword.equals(COMPAREWORD_BI ))
    	{
    		result = ps.biAnalyze(str);//"比"字句处理
    		}
    	else if(keyword.equals(COMPAREWORD_ZUI))
    	{
    		result = ps.superiorAnalyze(str);//"最"字句处理
    		}
    	else if(keyword.equals(COMPAREWORD_YIYANG))
    	{
	...
	public LinkedList<SentiResult> equalityAnalyze(String review) {
		...
		try {
			...
			for(int j = 0; j < subSentence.size(); j++)
			{
				...
				else if(subSentence.get(j).getSrcWord().equals(COMPAREWORD_YIYANG_A)&&foundC){
					foundY = true;
					posy = j;
				}
			...
			for(int neg = 0; neg < negPos.size(); neg++)
			{//否定词与“一样”前的距离在5以内，否则不考虑
				if(Math.abs(posy - negPos.get(neg)) < 5){
					foundN = true;
					negcount++;
				}
			}
		...
		try {
			...
			for(int j = 0; j < subSentence.size(); j++)
			{
					...
					{	//比较词之后到逗号之前的分句中的情感词
						...
						//System.out.println("情感词为"+subSentence.get(j).getSrcWord()+"情感词的倾向为"+q);
						sentiword = subSentence.get(j).getSrcWord();
						}
					}

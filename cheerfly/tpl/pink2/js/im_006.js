    ...
    init: function(options, sta, dataObj) {
        ...
        function initContent(json) {
            ...
            len = picBoard.length; j < len; j++) {
                ...
                var imgList = '';
				var _isMeFollow = picBoard[j].isMe == 1 ? '<div class="follow"><a href="javascript:;" status="follow" class="longbutton concern jquery-option" onclick="$.Edit_Album('+picBoard[j].id+')" >编辑</a></div>':'<div class="follow"><a href="'+picBoard[j].url+'" status="follow" class="longbutton concern jquery-option">查看</a></div>';
                ...
                var _to = rows + "px";
                var inserDiv = ['<div id="' + autoLayout_album.divId + '" columns="' + cols + '" class="mbox" style="left:' + _le + ";top:" + _to + '; "><div class="followlist">', '<h3 class="mboxTitle"><span class="number floatright">共'+picBoard[j].img_count+'张</span><span class="t" title="d">'+picBoard[j].title+'</span></h3>', ''+picBoard[j].imgs+_isMeFollow+'</div></div>'].join("");

var hostName = "www.cheerfly.com/";
var domain = "http://www.cheerfly.com/";
var tplName = "pink2"; (function() {
    var a = {
        hostName: "",
        domain: "",
		tplName: "",
        ...
        addStyle: function() {
            ...
            if (a.isIE6 || a.isIE7) {
                ...
                e = ".imSubmitInputOff{cursor:pointer;border:0;padding:0;background:url("+a.domain+"tpl/"+a.tplName+"/images/"+"addcc_ok0.gif) no-repeat;visibility:visible;width:340px;height:52px;line-height:52px;margin:0 auto;vertical-align:bottom;}";
                b = ".imSubmitInput{cursor:pointer;border:0;padding:0;background: url("+a.domain+"tpl/"+a.tplName+"/images/"+"addcc_ok1.gif) no-repeat ; visibility:visible;height: 52px;line-height:52px;margin: 0 auto;vertical-align: text-bottom;width: 340px;}"
            }
        ...
        addLayout: function() {
            ...
            l.appendChild(c);
            var d = ['<form id="imPingForm" name="imPingForm" >', 
                     '<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                     '<input type="hidden" name="imgUrl" value="">', 
                     '<input type="hidden" name="pageUrl" value="'+a.locationHost+'">',
                     '<input id="button" type="button" value="" class="imSubmitInputOff" readonly="readonly" />', 
                     '<span id="imCheckAll" class="checkimg">选择全部</span>', 
                     "</form>"].join("");
            c.innerHTML = d;
            ...
            function j(s) {
                ...
                var r = a.domain + "collectpics.php";
             //   var r = a.domain + "collectpics.php?imgUrl="+q.join(",")+"&pageUrl=" + a.locationHost + "&cookie='" + a.getCookie() + "'";
             //   document.getElementById("imPingForm").append('<input type="hidden" name="imgUrl" value="'+q.join(",")+'">");
                if (q.length == 0) {
                    var o = ['<form id="imPingForm" name="imPingForm" >', 
                            // '<input type="hidden" name="imgUrl" value="'+q.join(",")+'">',
                             '<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                             '<input id="button" type="button" value="" class="imSubmitInputOff" readonly="readonly" />', 
                             '<span id="imCheckAll" class="checkedimg">选择全部</span>', "</form>"].join("");
                    document.getElementById("imTitle").innerHTML = o
                } else {
                    if (a.isSogou || a.isIE9 || a.isMaxthon) {
                        var o = ['<form id="imPingForm" name="imPingForm" onsubmit="javascript:function(){window.open(\'\',\'imPingTool\',\'width=790,height=490,scrollbars=yes\');}()"  action="' + r + '" method="post" target="imPingTool">', 
                                 '<input type="hidden" name="imgUrl" value="'+q.join(",")+'">',
                                 '<input type="hidden" name="pageUrl" value="'+a.locationHost+'">',
                                 '<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                                 '<input id="imSubmit" type="submit" value="" class="imSubmitInput" />', 
                                 '<span id="imCheckAll" class="checkimg">选择全部</span>', "</form>"].join("");
                        document.getElementById("imTitle").innerHTML = o
                    } else {
                        ...
                        var o = ['<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                                 '<input type="hidden" name="imgUrl" value="'+q.join(",")+'">',
                                 '<input type="hidden" name="pageUrl" value="'+a.locationHost+'">',
                                 '<input id="imSubmit" type="submit" value="" class="imSubmitInput" />', 
                                 '<span id="imCheckAll" class="checkimg" >选择全部</span>'].join("");
                        p.innerHTML = o
                    }
                }
                document.getElementById("imCheckAll").onclick = function() {
            ...
            function k(r) {
                ...
                if (v == "checkedimg") {
                    ...
                    var q = ['<form id="imPingForm" name="imPingForm" >', 
                             '<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                             '<input id="button" type="button" value="" class="imSubmitInputOff" readonly="readonly" />', 
                             '<span id="imCheckAll" class="checkimg">选择全部</span>', "</form>"].join("");
                    ...
                    var p = a.domain + "collectpics.php";
               //     var p = a.domain + "collectpics.php?imgUrl=" + a.selected.join(",") + "&pageUrl=" + a.locationHost + "&cookie='" + a.getCookie() + "'";
                    if (a.isSogou || a.isIE9 || a.isMaxthon) {
                        var q = ['<form id="imPingForm" name="imPingForm" onsubmit="javascript:function(){window.open(\'\',\'imPingTool\',\'width=980,height=490,scrollbars=yes\');}()"  action="' + p + '" method="post" target="imPingTool">', 
                                 '<input type="hidden" name="imgUrl" value="'+a.selected.join(",")+'">',
                                 '<input type="hidden" name="pageUrl" value="'+a.locationHost+'">',
                                 '<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                                 '<input id="imSubmit" type="submit" value="" class="imSubmitInput" />', 
                                 '<span id="imCheckAll" class="checkedimg">选择全部</span>', "</form>"].join("");
                        ...
                        var q = ['<a id="imClosePing" class="closewin" href="javascript:void(0);"></a>', 
                                 '<input type="hidden" name="imgUrl" value="'+a.selected.join(",")+'">',
                                 '<input type="hidden" name="pageUrl" value="'+a.locationHost+'">',
                                 '<input id="imSubmit" type="submit" value="" class="imSubmitInput" />', 
                                 '<span id="imCheckAll" class="checkedimg" >选择全部</span>'].join("");
                        u.innerHTML = q
                    }
                }
                document.getElementById("imCheckAll").onclick = function() {

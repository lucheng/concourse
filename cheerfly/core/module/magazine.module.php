...
class MagazineModule
{
	public function index()
	{
		...
		//专辑flash
//		$flash_album = FS("Magazine")->getFlashMagazines(4);
		
		//推荐专辑
//		$best_album = FS("Magazine")->getBestMagazines(6);
		
		//最新专辑作者
//		$new_users = FS("Magazine")->getNewUsers(6);
		
		//最热专辑作者
//		$hot_users = FS("Magazine")->getHotUsers(6);
		...
		/*
		$sql = 'SELECT COUNT(DISTINCT tid) FROM '.FDB::table('magazine_share');
		...
		if($count > 0)
		{
			$pager = buildPage('magazine/'.ACTION_NAME,$page_args,$count,$_FANWE['page'],40);
			$share_list = FDB::fetchAll('SELECT * FROM '.FDB::table('share').'  
				WHERE status = 1 AND type = \'magazine_item\''.$order.' LIMIT '.$pager['limit']);
		...
		include template('page/magazine/magazine_index');
		display();
	}
	...
	public function show()
	{
		...
		$album = FS("Magazine")->getAlbumById($id);
		if(empty($album))

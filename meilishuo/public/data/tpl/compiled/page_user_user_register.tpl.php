...
<div class="err_ulike">支持中文，不能以数字开头，由6~20个字符组成，中文算两个字符。</div>
...
<span>您也可以用以下方式登录：</span><? if(is_array($api_config)) { foreach($api_config as $key => $api) { ?><a class="o_icon" href="<?=$api['url']?>" style="background:url(<?=$_FANWE['site_root']?>login/<?=$key?>/icon_<?=$key?>.png) no-repeat left center;"><?=$api['short_name']?></a>
<? } } ?>
</div>
</div>
</div>

...
<img src="<?php echo getImgName($share['imgs'][0]['img'],"$width","$width"); ?>" class="lazyload">
<div class="sk" style="display:none;">
<div class="fc"><span><?=$share['collect_count']?></span><i></i></div>
<a href="<?php echo FU('book/cate',array("cate"=>$cate_item['cate_code'],"tag"=>$share_item['tag_encode'])); ?>" target="_blank" class="kw"><?=$share_item['tag_name']?></a>
...
<div class="share_who">
她们在分享<?=$cate_item['short_name']?>：<? if(is_array($cate['user'])) { foreach($cate['user'] as $user) { if($user_index > 20) { ?><?php break; } ?><?php $user_index++; ?><a target="_blank" href="<?php echo FU('u/index',array("uid"=>$user['uid'])); ?>"><img class="r3 icard lazyload" height="24" src="<?php echo avatar($user['uid'],'s',1);?>" alt="<?=$user['user_name']?>" /></a>
<? } } ?>
</div>
</div>

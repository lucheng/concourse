...
<table cellpadding="4" cellspacing="0" border="0" class="table-form">
	<tr>
		...
		<td><input type="text" class="textinput" name="name" id="name" value="<?php echo ($vo["name"]); ?>" style="width:400px;" /></td>
	</tr>
	<tr>
		<th><?php echo L("PARENT_CATE");?></th>
		<td>
			<select name="parent_id">
				<option value="0" <?php if($vo['parent_id'] == 0): ?>selected="selected"<?php endif; ?>><?php echo L("TOP_CATE");?></option>
				<?php if(is_array($cate_tree)): foreach($cate_tree as $key=>$cate_item): ?><option value="<?php echo ($cate_item["fid"]); ?>" <?php if($vo['parent_id'] == $cate_item['fid']): ?>selected="selected"<?php endif; ?>><?php echo ($cate_item["title_show"]); ?></option><?php endforeach; endif; ?>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<?php if($vo['fields']['logo'] != ''): ?><a href="__ROOT__/<?php echo ($vo["fields"]["logo"]); ?>" target="_blank" ><?php echo L("VIEW");?></a><?php endif; ?>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<?php if($vo['fields']['img'] != ''): ?><a href="__ROOT__/<?php echo ($vo["fields"]["img"]); ?>" target="_blank" ><?php echo L("VIEW");?></a><?php endif; ?>
		</td>
	</tr>
	<tr>
		<th><?php echo L("DESC");?></th>
		<td>
			<textarea name="desc" class="textinput areainput" rows="3"><?php echo ($vo["fields"]["desc"]); ?></textarea>
		</td>
	</tr>
	<tr>
		<th><?php echo L("SORT");?></th>
		<td>
			<input type="text" class="textinput" name="sort" id="sort" value="<?php echo ($vo["sort"]); ?>"  />
		</td>
	</tr>

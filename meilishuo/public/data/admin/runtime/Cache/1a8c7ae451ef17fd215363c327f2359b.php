...
<!-- Think 系统列表组件开始 -->
<table id="checkList" class="table-list list" cellpadding="0" cellspacing="0" border="0"><thead><tr><th width="50" class="first"><a href="javascript:sortBy('gid','<?php echo ($sort); ?>','index')" title="按照<?php echo L("ID");?><?php echo ($sortType); ?> "><?php echo L("ID");?><?php if(($order)  ==  "gid"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th width="~left" ><a href="javascript:sortBy('name','<?php echo ($sort); ?>','index')" title="按照<?php echo L("NAME");?><?php echo ($sortType); ?> "><?php echo L("NAME");?><?php if(($order)  ==  "name"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th ><a href="javascript:sortBy('type','<?php echo ($sort); ?>','index')" title="按照<?php echo L("TYPE");?><?php echo ($sortType); ?> "><?php echo L("TYPE");?><?php if(($order)  ==  "type"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th width="50" ><a href="javascript:sortBy('status','<?php echo ($sort); ?>','index')" title="按照<?php echo L("STATUS");?><?php echo ($sortType); ?> "><?php echo L("STATUS");?><?php if(($order)  ==  "status"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th width="100">操作</th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): ++$i;$mod = ($i % 2 )?><tr class="<?php if(($mod)  ==  "0"): ?>even<?php endif; ?>"><td class="first"><?php echo ($node["gid"]); ?></td><td align="left" ><span class="pointer" module="UserGroup" href="javascript:;" onclick="textEdit(this,'<?php echo (addslashes($node["gid"])); ?>','name')"><?php echo ($node["name"]); ?></span></td><td ><?php echo (getTypeName($node["type"])); ?></td><td ><span class="pointer" module="UserGroup" href="javascript:;" onclick="toggleStatus(this,'<?php echo (addslashes($node["gid"])); ?>','status')"><?php echo (getStatusImg($node["status"])); ?></span></td><td><a href="javascript:;" onclick="editData(this,'<?php echo ($node["gid"]); ?>','gid')"><?php echo L("EDIT");?></a>&nbsp;&nbsp; <?php echo (getDelLink($node["type"],$node['gid'])); ?>&nbsp;&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table>
<!-- Think 系统列表组件结束 -->

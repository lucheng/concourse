...
class TaobaoSubmit {
     ...
     * @param $taobao_config 基本配置信息数组
     * @return 要请求的参数数组
     */
	function buildRequestPara($para_temp,$taobao_config) {
		//除去待签名参数数组中的空值和签名参数
		...
		//生成签名结果
		$mysign = buildMysign($para_sort, trim($taobao_config['key']), strtoupper(trim($taobao_config['sign_type'])));
		
		//签名结果与签名方式加入请求提交参数组中
		$para_sort['sign'] = $mysign;
		$para_sort['sign_type'] = strtoupper(trim($taobao_config['sign_type']));
     ...
     * @return 提交表单HTML文本
     */
	function buildForm($gateway, $method, $button_name, $taobao_config) {
		
		$sHtml = "<form id='alipaysubmit' name='alipaysubmit' style='display:none;' action='".$gateway."' method='".$method."'>";
        $sHtml.= "<input type='hidden' name='appkey' value='".$taobao_config['app_key']."'/>";
        $sHtml.= "<input type='hidden' name='encode' value='".$taobao_config['encode']."'/>";

...
class NoteModule
{
	...
	private function show($current_type = '')
	{
		...
		$_FANWE['nav_title'] = preg_replace("/\[[^\]]+\]/i","",$share_detail['content']).' - '.lang('common','share');
        $_FANWE['seo_description'] = $_FANWE['nav_title'];
		...
		$share_detail = array_merge($share_detail,$share_dynamic);
		unset($share_dynamic);
		...
		//喜欢分享的会员
		$share_detail['collects'] = FS('Share')->getShareCollectUser($share_id,$_FANWE['uid']);

		//会员显示名称
		$user_show_name = FS('User')->getUserShowName($share_detail['uid']);

		//分享标签
		$share_tags = FS('Share')->getShareTagsCache($share_id);
		$share_tags = $share_tags['user'];
		...
		switch($current_type)
		{
			...
			default:
				...
				if($share_detail['parent_id'] > 0 && $share_detail['base_id'] > 0)
				{
					...
					$dynamic = FS('Share')->getShareDynamic($share_detail['base_id']);
					$parent_share = FS('Share')->getShareDetail($share_detail['base_id']);
					$parent_share = array_merge($parent_share,$dynamic);

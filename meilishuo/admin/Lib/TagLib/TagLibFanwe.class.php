...
class TagLibFanwe extends TagLib
{
	protected $tags =  array(
		...
		'list'=>array('attr'=>'id,datasource,pk,style,name,action,checkbox,actionlist,show,action_width,action_align,nosort','level'=>1,'close'=>0),
    );
    /**
     +----------------------------------------------------------
     * editor标签解析 插入可视化编辑器
    ...
    public function _list($attr)
    {
        ...
        foreach($fields as $field) {//显示指定的字段
            ...
            if(isset($showname[1])) {
                $parseStr .= '<th width="'.$showname[1].'" '.$first.'>';
            }else {
                $parseStr .= '<th '.$first.'>';
            }
			$first = '';
        ...
        $parseStr .= '</tr></thead>';
        $parseStr .= '<tbody><volist name="'.$datasource.'" id="'.$name.'" mod="2"><tr class="<eq name="mod" value="0">even</eq>">';	//支持鼠标移动单元行颜色变化 具体方法在js中定义

...
<div class="col2">
        <div class="list1">
          <div class="list1_title"><a href="/">Home</a> > <?php echo ($disp_text); ?> Products</a>
	</volist></div>
          <div class="sort_by">
              <form method="get"><label for="disp-order-sorter">Sort by: </label>
              <select name="order" onchange="this.form.submit();" id="disp-order-sorter">
                <option value="id" <?php if(($_REQUEST['order'])  ==  "id"): ?>selected<?php endif; ?>>Sort by id</option>
                <option value="name" <?php if(($_REQUEST['order'])  ==  "name"): ?>selected<?php endif; ?>>Sort by name</option>
                <option value="price" <?php if(($_REQUEST['order'])  ==  "price"): ?>selected<?php endif; ?>>Sort by price</option>
                <option value="dateline" <?php if(($_REQUEST['order'])  ==  "dateline"): ?>selected<?php endif; ?>>Sort by update</option>              </select>
                 <select name="sort"  onchange="this.form.submit();" >
              <option value="0" <?php if(($_REQUEST['sort'])  ==  "0"): ?>selected<?php endif; ?>>Descending</option><option value="1" <?php if(($_REQUEST['sort'])  ==  "1"): ?>selected<?php endif; ?>>Ascending</option>              </select>
            </form>
          </div>
             <script language="javascript" type="text/javascript">
...
window.location.href=$("#spage").val(); 
  ...
  <tr>
    ...
    <td align="right"><?php echo ($page); ?></td>
          ...
          <div class="gride">
          <div class="gride_title"><?php echo ($disp_text); ?> Products</div>
          <div class="gride_content">
            <div class="gride_pai">
              <ul>
                <li>Price：
                 <a href="?pr=0&pr2=50" <?php if(($_REQUEST['pr2'])  ==  "50"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>0-50</a>
                <a href="?pr=50&pr2=100" <?php if(($_REQUEST['pr2'])  ==  "100"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>50-100</a>
                <a href="?pr=100&pr2=200" <?php if(($_REQUEST['pr2'])  ==  "200"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>100-200</a>
                <a href="?pr=200&pr2=300" <?php if(($_REQUEST['pr2'])  ==  "300"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>200-300</a>
                </li>
                <li>Type：
                <a href="?type=ishot" <?php if(($_REQUEST['type'])  ==  "ishot"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>Bestsellers</a>
                <a href="?type=isnew" <?php if(($_REQUEST['type'])  ==  "isnew"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>New</a>
                <a href="?type=isrec" <?php if(($_REQUEST['type'])  ==  "isrec"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>Recommended</a>
                <a href="?type=isprice" <?php if(($_REQUEST['type'])  ==  "isprice"): ?>class="click" style="color:#FFFFFF"<?php endif; ?>>Specials</a>
                </li>
              </ul>
            </div>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><div class="g_pro">
              ...
              <div class="g_pro_name"><a href="<?php echo build_url($vo,'pro_url');?>"><?php echo build_url($vo,'pro_name');?></a>&nbsp;&nbsp;&nbsp;<span>ID:<?php echo ($vo["id"]); ?></span><br /></div>
              <div class="g_pro_pri">
               <?php echo build_url($vo,'pro_price');?>
              <br /><br /><a href="<?php echo build_url($vo,'pro_url');?>"><img src="../Public/images/button_in_cart.gif" alt="" border="0" /></a>
              </div>
            </div><?php endforeach; endif; else: echo "" ;endif; ?>

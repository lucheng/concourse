...
class Page extends Think {
	...
	protected $config  =	array('header'=>'Records','prev'=>'Prev','next'=>'Next','first'=>'First','last'=>'Last','theme'=>' %totalRow% %header% %nowPage%/%totalPage% pages %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%');
	...
	public function show() {
		...
		if($nowCoolPage == 1){
			...
			$prePage = "[<a href='".$url.$strpath.$p.$strpath."$preRow.html' >Prev&nbsp;".$this->rollPage."Page</a>]";
		...
		if($nowCoolPage == $this->coolPages){
			...
			$nextPage = "[<a href='".$url.$strpath.$p.$strpath."$nextRow.html' >Next&nbsp;".$this->rollPage."Page</a>]";

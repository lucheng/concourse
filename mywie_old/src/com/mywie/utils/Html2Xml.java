...
class light_html2xml {
	...
	private static String Html2Xml(String s) {
		...
		for (int i = start; i < limit
				&& ((r2.equals("") && r.equals("")) || !opentags.empty()); i++) {
			if (r.length() > 10240) {
				r2 += r;
				r = "";
			}
			...
			case tag:
				...
				if (c == '>') {
					...
					if (!tagname.equals("")) {
						...
						if (autoclosetags.containsKey(tagname)
								&& !opentags.empty()) {
							String prevtag = opentags.peek();
							if (autoclosetags.get(tagname).contains(prevtag)) {
								opentags.pop();
				...
				cs = "" + c;
				name += (Character.isLetterOrDigit(c) && name != "")
						|| Character.isLetter(c) ? cs : (name.equals("") ? "_"
						: (c == '-' ? "-" : (!name.equals("_") ? "_" : "")));
				break;
			case endtag:
				if (c == '>') {
				...
				cs = "" + c;
				name += Character.isLetterOrDigit(c) ? cs
						: !name.equals("_") ? "_" : "";
				break;
			case attrtext:
				if (c == attrdelim
						|| (Character.isWhitespace(c) && attrdelim == ' ')) {
					if (attrname.equals("_")) {
						while (attrnames.contains(attrname)) {
							attrname += "_";
						}
			...
			case hexaentity:
				...
				entvalue = entvalue
						* 16
						+ (Character.isDigit(c) ? c - '0' : Character
								.toUpperCase(c) - 'A');
				break;
			case namedentity:
				if (c == ';') {
					String ent;
					name = name.toLowerCase();
					if (name.equals("amp") || name.equals("lt")
							|| name.equals("gt") || name.equals("quot")
							|| name.equals("apos")) {
						ent = "&" + name + ";";
						name = "";
						if (prevstate == states.text) {

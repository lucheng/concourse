...
//					upyun.mkDir(servletContext.getRealPath(goodsImage.getSourceImagePath()));
				}
				if (!bigGoodsImageParentFile.isDirectory()) {
					bigGoodsImageParentFile.mkdirs();
//					upyun.mkDir(servletContext.getRealPath(goodsImage.getBigImagePath()));
				}
				if (!smallGoodsImageParentFile.isDirectory()) {
					smallGoodsImageParentFile.mkdirs();
//					upyun.mkDir(servletContext.getRealPath(goodsImage.getSmallImagePath()));
				}
				if (!thumbnailGoodsImageParentFile.isDirectory()) {
					thumbnailGoodsImageParentFile.mkdirs();
//					upyun.mkDir(servletContext.getRealPath(goodsImage.getThumbnailImagePath()));
				}*/
				
				upyun.writeFile(setting.getImageUploadRealPath() + "/source/" + uuid + "."+ goodsImage.getSourceImageFormatName() , goodsImageFile, true); //可自动创建父级目录（最多10级）
